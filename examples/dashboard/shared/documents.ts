export type DocumentStatus = "Done" | "In Process";
...
export function computeStats(docs: Document[]) {
  const done = docs.filter((d) => d.status === "Done").length;
  const inProcess = docs.filter((d) => d.status === "In Process").length;
  const reviewers = new Set(docs.map((d) => d.reviewer)).size;
  const types = new Set(docs.map((d) => d.type)).size;
  return { total: docs.length, done, inProcess, reviewers, types };
}
